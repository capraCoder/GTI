"""
GTI Classifier v2.1
Game Theory Index — Extended coverage with edge case detection
"""
...
# ============================================================
# v2.1 EDGE CASE DETECTION
# ============================================================

def detect_network_effects(case: Dict) -> bool:
    """Detect coordination games with network effects."""
    keywords = ['network effect', 'standard', 'format', 'compatibility',
                'adopt', 'installed base', 'platform', 'ecosystem',
                'vhs', 'betamax', 'blu-ray', 'hd-dvd', 'format war',
                'winner take', 'tipping point', 'critical mass',
                'network externality', 'bandwagon', 'momentum']
    text = str(case).lower()
    return any(kw in text for kw in keywords)


def detect_commitment_problem(case: Dict) -> bool:
    """Detect games with commitment/credibility issues."""
    keywords = ['commitment problem', 'cannot credibly', 'credibly commit',
                'trust but verify', 'irreversible', 'binding agreement',
                'disarm first', 'who moves first', 'sequential exchange',
                'hold-up', 'holdup', 'reneging', 'renege']
    text = str(case).lower()
    return any(kw in text for kw in keywords)


def detect_two_sided_trust(case: Dict) -> bool:
    """Detect simultaneous trust games (both parties at risk)."""
    keywords = ['both sides', 'mutual trust', 'simultaneous risk',
                'host and guest', 'buyer and seller', 'two-sided',
                'marketplace', 'escrow', 'intermediary',
                'both must trust', 'neither moves first',
                'strangers', 'platform trust']
    text = str(case).lower()
    return any(kw in text for kw in keywords)


def detect_repeated_game(case: Dict) -> bool:
    """Detect repeated game dynamics."""
    keywords = ['repeated', 'ongoing', 'long-term', 'reputation',
                'future interaction', 'shadow of future', 'tit for tat',
                'retaliation', 'punishment', 'cooperate again',
                'every day', 'each round', 'iterated']
    text = str(case).lower()
    time_info = case.get('time', {})
    is_repeated = time_info.get('repetition') in ['finite', 'infinite']
    return is_repeated or any(kw in text for kw in keywords)
...
def classify_case(case: Dict) -> ClassificationResult:
    ...
    # v2.1: Check for trust-game flags (check before network-effects due to overlap)
    if any(f in ['trust-game', 'trust game', 'two-sided-trust', 'platform-mediated'] for f in flags):
        return ClassificationResult(
            game_type=GameType.TRUST_GAME,
            confidence=80,
            reasoning="Trust game - mutual trust required, often platform-mediated",
            fingerprint={},
            subcategory="two_sided"
        )

    # v2.1: Check for coordination/network-effects flags
    if any(f in ['coordination-game', 'network-effects', 'network effects', 'format war'] for f in flags):
        return ClassificationResult(
            game_type=GameType.COORDINATION,
            confidence=80,
            reasoning="Coordination game with network effects - standard/platform adoption",
            fingerprint={},
            subcategory="network_effects"
        )

    # v2.1: Check for commitment-related flags
    if any(f in ['commitment-problem', 'commitment problem', 'credibility'] for f in flags):
        return ClassificationResult(
            game_type=GameType.SEQUENTIAL_BARGAINING,
            confidence=75,
            reasoning="Commitment/credibility problem in sequential exchange",
            fingerprint={},
    ...
    # v2.1: Edge case detection before giving up on no-matrix cases
    if not matrix:
        # Check for network effects → Coordination variant
        if detect_network_effects(case):
            return ClassificationResult(
                game_type=GameType.COORDINATION,
                confidence=75,
                reasoning="Network effects create coordination incentives around standards/platforms",
                fingerprint={},
                subcategory="network_effects"
            )

        # Check for commitment problems → Sequential Bargaining
        if detect_commitment_problem(case):
            return ClassificationResult(
                game_type=GameType.SEQUENTIAL_BARGAINING,
                confidence=70,
                reasoning="Commitment problem in sequential exchange or negotiation",
                fingerprint={},
                subcategory="commitment"
            )

        # Check for two-sided trust → Trust Game variant
        if detect_two_sided_trust(case):
            return ClassificationResult(
                game_type=GameType.TRUST_GAME,
                confidence=70,
                reasoning="Two-sided trust problem, often with platform intermediary",
                fingerprint={},
                subcategory="two_sided"
            )
    ...
    result = classify_2x2_symmetric(fingerprint)

    # v2.1: If symmetric classification returns Unknown, try edge case detection
    if result.game_type == GameType.UNKNOWN:
        if detect_network_effects(case):
            return ClassificationResult(
                game_type=GameType.COORDINATION,
                confidence=75,
